...
setup() {
    ...
    export NOTIFY_CMD="tests/audit/notify_mock.sh"
...
@test "ssh-log-watcher processes SSH accepted connections" {
    ...
    run bash -c "echo '$log_line' | $SCRIPT_DIR/ssh-log-watcher.sh"
    [ "$status" -eq 0 ]
...
Jan 12 10:38:30 bastion sshd[12354]: Failed password for user2 from 192.168.1.2 port 22222 ssh2"
    ...
    run bash -c "echo '$log_lines' | $SCRIPT_DIR/ssh-log-watcher.sh"
    [ "$status" -eq 0 ]
